...
tweets_emoticons_sentiment <- tweets_emoticons_sentiment %>%

  mutate(   total_chars = nchar(text)
          , total_chars_minus_emotics = nchar(text) - emoticon_count)
...
optimized_for_viz <- tweets_emoticons_sentiment %>%
  ...
  select(id, user
         ...
         , has_hashtag, is_retweet, is_response, total_chars, total_chars_minus_emotics)

optimized_for_viz$type <- "tweet"
optimized_for_viz[optimized_for_viz$is_response==1, 20] <- "reply"
optimized_for_viz[optimized_for_viz$is_retweet==1, 20] <- "RT"

optimized_for_viz$type <- as.factor(optimized_for_viz$type)
optimized_for_viz$type = factor(optimized_for_viz$type,levels(optimized_for_viz$type)[c(3,1,2)])
...
## plots
p <- ggplot(data = optimized_for_viz, aes(x = timeonly)) +
